...
먼저 개발자가 구현을 마친 뒤 작성자 측에서 AI 리뷰와 개선 루프를 돈다.
이때 같은 대화 맥락이 아니라 다른 컨텍스트에서 리뷰하는 것이 좋다.
subagent, 별도 프로세스, 다른 모델, 또는 Claude/Codex 같은 다른 AI 제품을 사용할 수 있다.

목표는 Major 이상의 결함이 나오지 않을 때까지 반복 리뷰와 개선 루프를 도는 것이다.
이 단계에서 많은 결함은 개발팀 내부에서 걸러진다.
...
다만 AI 3자 리뷰가 Human 리뷰를 대체한다는 뜻은 아니다.
Human 리뷰가 필요한 변경은 사람이 최종 코드 리뷰를 해야 한다.

Human 리뷰 비율은 일괄적으로 정하기 어렵다.
복잡하고 큰 운영 시스템은 Human 리뷰 비율이 높아질 것이고, 작고 실패 비용이 낮은 시스템은 거의 없을 수도 있다.

중요한 기준은 운영 중 문제가 발생했을 때 담당자가 대응할 수 있는가다.
버그나 장애가 발생했는데 담당자가 코드 구조를 이해하지 못하고 AI에게 "고쳐줘"만 반복하면, AI가 맥락을 못 잡거나 다른 버그를 만들 수 있다.
반대로 담당자가 전체 구조와 코드 흐름, 영향 범위를 어느 정도 알고 있으면 AI와 함께 원인을 좁히고, AI를 통해서든 직접 수정하든 빠르게 장애를 해소할 수 있다.

따라서 Human 리뷰의 기준은 "모든 PR을 동일 비율로 사람이 본다"가 아니다.
